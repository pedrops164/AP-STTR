...
def get_args_parser():
    ...
    parser.add_argument('--in_content_folder', default=None, type=str)
    parser.add_argument('--style_folder', default=None, type=str)
...
def main(args):
    ...
    save_name="{}_noLN_contentMstyleM_dataN_enorm{}_dnorm{}_tnorm{}_cbl{}_sbl{}_batch{}_k{}_s{}_lrb{}_lr{}_cw{}_sw{}_tw{}_elyers{}_dlyers{}_h{}_maxsize{}".\
                                  format(args.model_type,args.enorm,args.dnorm,args.tnorm,args.cbackbone_layer,args.sbackbone_layer,args.batch_size,args.fold_k,\
                                      args.fold_stride,args.lr_backbone,args.lr,\
                                      args.content_loss_coef,args.style_loss_coef,args.tv_loss_coef,args.enc_layers,args.dec_layers,\
                                         args.nheads,args.img_size) 
        
    # set logger
    if args.eval:
        output_dir = Path(os.path.join(args.output_dir+"_eval",save_name))
    else:
        output_dir = Path(os.path.join(args.output_dir,save_name))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % 10 == 0:
                checkpoint_paths.append(output_dir  / "checkpoint" / f'checkpoint{epoch:04}.pth')
